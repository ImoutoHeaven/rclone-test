...
### 关于 superfile2 返回的 “md5”

在实际抓包和日志中，我们观察到：

- 对于同一批测试文件，同一个分片序号（`partseq`）在不同文件上，本地分片内容的 MD5 各不相同；
- 但 Baidu `superfile2` 响应中该 `partseq` 的 `md5` 值在不同文件之间完全一致；
- 对于同一个文件，不同 `partseq` 又对应不同的服务端 “md5”。

这表明：

- `superfile2` 返回的 `md5` 更像是“按分片序号生成的内部块标识/伪 md5”，而非对分片原始数据的真实 MD5；
- Baidu `create` 接口在实践中既接受“本地真实块 MD5 列表”，也接受“superfile2 返回的这套伪 md5 列表”，并不会强制要求两者一致；
- errno=10 的产生主要与 Baidu 内部对 `uploadid + partseq` 的处理/时序/节点状态有关，而不是我们提交的 `block_list` 文本。

因此：

- `serverside_md5_override` 只能在少数情况下作为兼容/救火选项，无法从根本上解决 errno=10，也不能提供端到端的数据完整性保证；
- 我们不会基于 `superfile2` 的 `md5` 字段实现所谓“分片级校验”，避免引入大量伪错误；
- 后端默认仍声明不支持 HashMD5，完整性需要依赖本地自算哈希或额外工具，在协议层通过重试、动态节点选择与合理的超时策略尽量规避 Baidu 侧的不稳定行为。
